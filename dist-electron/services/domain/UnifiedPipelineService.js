...
const DeepgramLanguageSupport_1 = require("./DeepgramLanguageSupport");
...
class UnifiedPipelineService extends events_1.EventEmitter {
    constructor(audioConfig, deepgramConfig, openaiConfig, languageConfig = { sourceLanguage: 'en', targetLanguage: 'ja' }) {
        ...
        this.audioFrameCount = 0; // 音声フレームカウンター
     ...
     */
    async translateParagraph(paragraphId, sourceText, sourceLanguage, targetLanguage, correlationId) {
        try {
            ...
            // パラグラフ翻訳用のプロンプトを生成
            const systemPrompt = (0, LanguageConfig_1.getTranslationPrompt)(sourceLanguage, targetLanguage);
            ...
            // エラーイベントを発行
            this.emitError('PARAGRAPH_TRANSLATION_FAILED', error instanceof Error ? error.message : 'Failed to translate paragraph', correlationId, {
                recoverable: true,
                details: { paragraphId }
            });
        }
    ...
    async startListening(sourceLanguage = 'en', targetLanguage = 'ja', correlationId) {
        ...
        if (!(0, DeepgramLanguageSupport_1.isDeepgramSupported)(sourceLanguage)) {
     ...
     */
    async updateLanguages(sourceLanguage, targetLanguage) {
    ...
    */
    ...
    async executeTranslation(queuedTranslation) {
        ...
        try {
            ...
            */
            ...
            this.emitError('TRANSLATION_FAILED', `Failed to translate segment: ${error instanceof Error ? error.message : 'Unknown error'}`, this.currentCorrelationId || 'unknown', {
                recoverable: true,
                details: {
                    segmentId,
                    sourceLanguage: this.sourceLanguage,
     ...
     * Emit error event
     */
    handleTranslationQueueFailure(translation, error) {
        const kind = translation.kind ?? 'realtime';
        if (kind === 'history' || kind === 'paragraph') {
            this.handleHistoryTranslationFailure(translation, error);
            return;
        }
        this.handleRealtimeTranslationFailure(translation.segmentId, translation.sourceText, error);
    }
    handleRealtimeTranslationFailure(segmentId, sourceText, error) {
    ...
    handleHistoryTranslationFailure(translation, error) {
        const reason = error instanceof Error ? error.message : String(error);
    ...
    createFallbackTranslationText(sourceText, reason) {
...
${sourceText}`;
    ...
    emitError(code, message, correlationId, options) {
    ...
    async executeHistoryTranslation(queuedTranslation) {
        ...
        try {
            ...
            const sourceName = LanguageConfig_1.SUPPORTED_LANGUAGES[this.sourceLanguage].name;
            const targetName = LanguageConfig_1.SUPPORTED_LANGUAGES[this.targetLanguage].name;
