...
export class DataPersistenceService {
   ...
   * 同じ日の既存セッションを検索
   */
  public async findTodaySession(courseName: string, dateStr: string): Promise<{ sessionNumber: number; sessionId: string; folderName: string } | null> {
    try {
   ...
   */
  async getAvailableSessions(courseName?: string, limit: number = 20): Promise<Array<{ courseName: string; sessions: SessionMetadata[] }>> {
    try {
      ...
      const courseSessionsPromises = targetCourses.map(async (course) => {
        try {
          ...
          const sessions = await Promise.all(
            validSessions.map(async (sessionFolder) => {
              try {
          ...
          const validMetadata = sessions.filter((s): s is SessionMetadata => s !== null);
