...
function setupWindowControls(): void {
  ...
  ipcMain.handle('window:toggleHistory', async () => {
    const isVisible = windowRegistry.toggleHistory();
    return isVisible;
  });

  // Toggle summary window
  ipcMain.handle('window:toggleSummary', async () => {
    const isVisible = windowRegistry.toggleSummary();
    return isVisible;
  });
    ...
    try {
      ...
      if (command?.command === 'getFullHistory') {
        if (advancedFeatureService) {
          ...
          const data = {
            entries: translations.map((t: any) => ({
              id: t.id,
            ...
            metadata: {
              ...
              totalWords: translations.reduce((sum: number, t: any) => 
    ...
    try {
      switch (domainCommand.type) {
        ...
        case 'translateParagraph':
          if (pipelineService) {
            // パラグラフの再翻訳を実行
            await pipelineService.translateParagraph(
              domainCommand.params.paragraphId,
              domainCommand.params.sourceText,
              domainCommand.params.sourceLanguage,
              domainCommand.params.targetLanguage,
              domainCommand.correlationId
            );
