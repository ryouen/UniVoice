...
export class RealtimeDisplayManager {
  ...
  private updateSegmentStates(): void {
    // active以外はfadingへ。削除はpruneSegmentsが担当
    this.segments.forEach(s => {
      if (s.id !== this.currentSegmentId && s.status !== 'completed') {
        s.status = 'fading';
      }
    });
    this.emitUpdate();
  ...
  /**
   * 表示セグメントを安全に間引く。
   * - completed かつ 最小表示時間を満たしたものから優先削除
   * - active/fading は触らない（翻訳の整合性保持）
   */
  private pruneSegments(now: number): void {
    if (this.segments.length <= this.maxDisplaySegments) return;

    // 削除候補: completed && 最小表示時間満了
    const removableIdxs = this.segments
      .map((s, i) => ({ s, i }))
      .filter(({ s }) => {
        const displayTime = now - (s.displayStartTime || s.timestamp);
        return s.status === 'completed' && displayTime > this.minDisplayTimeMs;
      })
      .map(({ i }) => i);

    // 削除すべき数
    const needRemove = this.segments.length - this.maxDisplaySegments;
    if (needRemove <= 0) return;

    // 翻訳の最低保持（1.5s）に抵触しないかを確認
    const canRemoveIdxs: number[] = [];
    for (const idx of removableIdxs) {
      if (canRemoveIdxs.length >= needRemove) break;
      const seg = this.segments[idx];
      if (seg.translation && seg.translationStartTime) {
        const tDisp = now - seg.translationStartTime;
        if (tDisp < this.translationDisplayTimeMs) continue; // まだ保持
      }
      canRemoveIdxs.push(idx);
    }

    if (canRemoveIdxs.length === 0) return;

    // 先頭（古い）から削除。indexずれ回避のため降順でsplice
    canRemoveIdxs.sort((a, b) => b - a).forEach(i => this.segments.splice(i, 1));
  }
  ...
  private updateDisplayStates(): void {
    ...
    // セグメントの削除は pruneSegments に委譲
    this.pruneSegments(now);
    // pruneSegmentsが削除を行った場合、emitUpdateは内部で呼ばれるので追加の更新は不要
