...
export const useUnifiedPipeline = (options: UseUnifiedPipelineOptions = {}) => {
  ...
  const handleTranslationTimeout = useCallback((segmentId: string) => {
    ...
    // Also update RealtimeDisplayService for 3-line display
    if (realtimeDisplayServiceRef.current) {
      realtimeDisplayServiceRef.current.updateTranslation('[翻訳タイムアウト]');
      // 3段階表示の現在のセグメントを完了として、次の処理に進めるようにする
      realtimeDisplayServiceRef.current.completeCurrentSegment();
    }
  ...
  const handlePipelineEvent = useCallback((event: PipelineEvent) => {
    ...
    switch (event.type) {
      case 'asr':
        ...
        // Update display manager (SyncedRealtimeDisplayManager internally filters interim results)
      ...
      case 'translation':
        ...
        if (event.data.segmentId && event.data.segmentId.startsWith('history_')) {
          console.log('[useUnifiedPipeline] History translation received:', event.data.segmentId);
          
          // history_プレフィックスを削除して元のsegmentIdを取得
          const originalSegmentId = event.data.segmentId.replace('history_', '');
          
          // segmentIdからcombinedIdを取得
          const combinedId = segmentToCombinedMap.current.get(originalSegmentId) || originalSegmentId;
          const translationText = event.data.translationText || event.data.content;
          
          console.log('[useUnifiedPipeline] Mapping history translation:', {
            historySegmentId: event.data.segmentId,
            originalSegmentId,
            combinedId,
            hasMapping: segmentToCombinedMap.current.has(originalSegmentId)
          });
          
          // 高品質翻訳を保存
          if (translationText) {
            highQualityTranslationsRef.current.set(combinedId, translationText);
            console.log('[useUnifiedPipeline] High-quality translation stored:', combinedId, translationText.substring(0, 50));
            
            // FlexibleHistoryGrouperの内部状態も更新（Phase 1修正）
            if (historyGrouperRef.current) {
              historyGrouperRef.current.updateSentenceTranslation(combinedId, translationText);
              console.log('[useUnifiedPipeline] Updated FlexibleHistoryGrouper translation:', combinedId);
            }
