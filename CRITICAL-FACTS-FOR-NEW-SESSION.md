...
*最終更新: 2025-09-23（透過ウィンドウフォーカス問題解決・UniVoice.tsx Phase 1-2完了）*
...
3. ✅ UniVoice.tsx Phase 1-2完了（モーダル分離、質問セクション分離）
4. 🚧 UniVoice.tsx Phase 3-4（メモリスト分離、Context化）
5. SessionStorageServiceの統合（設定データ永続化）
6. プログレッシブ要約のUI修正
7. レポート表示UI（生成は動作中）
8. 語彙テーブルUI（抽出は動作中）
