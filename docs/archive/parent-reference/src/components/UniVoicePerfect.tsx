...
// Phase 2統合用のpropsインターフェース
interface UniVoicePerfectProps {
  // Phase 2: Current blockに実データを注入
  currentOriginalOverride?: string;
  currentTranslationOverride?: string;
  realtimeSegmentsOverride?: any[]; // リアルタイム複数行表示用
  // Phase 3: その他のブロックもオーバーライド可能
...
export const UniVoicePerfect: React.FC<UniVoicePerfectProps> = ({
  ...
  const [sectionHeights, setSectionHeights] = useState<SectionHeights>(() => {
    ...
    return saved ? JSON.parse(saved) : { history: 30, summary: 24, input: 20 };
  });
  ...
  // パイプライン接続（新useUnifiedPipelineを内部で使用）
  const {
    isRunning,
    error: pipelineError,
    currentOriginal: pipelineCurrentOriginal,
    currentTranslation: pipelineCurrentTranslation,
    history: segments,
    summaries,
    startFromMicrophone: startPipeline,
    stop: stopPipeline,
    startFromFile,
    translateUserInput: translateUserInputPipeline,
    // exportSessionは新APIにはないのでフォールバック実装を使用
    clearHistory: clearState
  } = useUnifiedPipeline();
  
  // isConnectedは常にtrueとして扱う
  const isConnected = true;
  const metrics = null; // metricsは新APIでは提供されない
  
  // propsよりもパイプラインからのデータを優先
  const currentOriginal = currentOriginalOverride || pipelineCurrentOriginal || '';
  const currentTranslation = currentTranslationOverride || pipelineCurrentTranslation || '';
  ...
  useEffect(() => {
    if (currentOriginal) {
      ...
      setCurrentDisplay(prev => ({
        ...prev,
        original: {
          oldest: realtimeBuffer.originalChunks[0] || '',
          older: realtimeBuffer.originalChunks[1] || '',
          recent: currentOriginal
        }
      }));
    }
  }, [currentOriginal, realtimeBuffer.originalChunks]);
  ...
  useEffect(() => {
    if (currentTranslation) {
      ...
      setCurrentDisplay(prev => ({
        ...prev,
        translation: {
          oldest: realtimeBuffer.translationChunks[0] || '',
          older: realtimeBuffer.translationChunks[1] || '',
          recent: currentTranslation
        }
      }));
    }
  }, [currentTranslation, realtimeBuffer.translationChunks]);
  
  // historyOverrideが提供されたら使用
  useEffect(() => {
    if (_historyOverride && _historyOverride.length > 0) {
      setHistoryEntries(_historyOverride);
    }
  }, [_historyOverride]);
  ...
  const startSession = async () => {
    ...
    // 音声許可を先に取得してからパイプライン開始（o3-pro推奨）
    try {
      console.log('[UniVoice] マイクアクセス許可を要求中...');
      // 先に音声キャプチャを開始（ユーザージェスチャーのコンテキスト内で実行）
      await startCapture();
      console.log('[UniVoice] マイクアクセス許可取得成功');
      
      console.log('[UniVoice] パイプライン開始中...');
      await startPipeline();
      console.log('[UniVoice] セッション開始成功');
    } catch (error: any) {
      console.error('[UniVoice] セッション開始失敗:', error);
        ...
        <div style={{ height: 'calc(100vh - 50px)', display: 'flex', flexDirection: 'column', position: 'relative' }}>
          ...
          <div style={{
            ...
            <div style={{
              ...
              <div id="currentOriginal" className="current-text" style={{
                fontSize: '20px',
                lineHeight: '1.6',
                color: '#111',
                fontWeight: 400,
                ...
                <span style={{ opacity: 0.3 }}>{currentDisplay.original.oldest} </span>
                <span style={{ opacity: 0.6 }}>{currentDisplay.original.older} </span>
                <span style={{ opacity: 1, fontWeight: 500 }}>{currentDisplay.original.recent}</span>
              </div>
            </div>
            <div style={{
              flex: 1,
              ...
              <div id="currentTranslation" className="current-text translation" style={{
                fontSize: '20px',
                lineHeight: '1.6',
                color: '#0066cc',
                fontWeight: 400,
                ...
                <span style={{ opacity: 0.3 }}>{currentDisplay.translation.oldest}</span>
                <span style={{ opacity: 0.6 }}>{currentDisplay.translation.older}</span>
                <span style={{ opacity: 1, fontWeight: 500 }}>{currentDisplay.translation.recent}</span>
              </div>
            </div>
          </div>
