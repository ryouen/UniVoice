 ...
 * 
 * test-20min-production.jsベースの詳細出力版
 * ①～⑥の各段階で詳細なログと出力を記録
 * 
...
// ========== 設定 ==========
const TEST_NAME = '20min-production-detailed';
...
const SUMMARY_INTERVAL_MINUTES = 10;
...
function connectDeepgram() {
  ...
  ws.on('open', () => {
    ...
    console.log('📡 WebSocket URL:', DEEPGRAM_WS_URL.replace(process.env.DEEPGRAM_API_KEY, '***'));
    ...
    try {
      ...
      if (msg.type === 'Results') {
        ...
        if (isFinal) {
          ...
          // 🎯 ASR結果をコンソールに明確表示（ユーザー視認性向上）
          const time = new Date().toISOString().split('T')[1].split('.')[0];
          console.log(`\n🎤 [${time}] ASR FINAL (${(confidence * 100).toFixed(1)}%): "${text}"`);
          ...
          // 🎯 ASR Interim結果をコンソールに表示（リアルタイム確認用）
          if (text.length > 3) {  // 短すぎるものは除外
            const time = new Date().toISOString().split('T')[1].split('.')[0];
            console.log(`💬 [${time}] ASR interim (${(confidence * 100).toFixed(1)}%): "${text}"`);
          }
...
function startAudioStream() {
  ...
  console.log(`📊 Frame settings: ${FRAME_MS}ms per frame, ${FRAME_SIZE} bytes per frame`);
  console.log(`⏱️  Expected stream duration: ${(Math.floor(audioBuffer.length / FRAME_SIZE) * FRAME_MS / 1000).toFixed(1)}s`);
