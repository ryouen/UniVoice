 ...
 */
export async function generateVocabulary(summariesText: string): Promise<any[]> {
 ...
 */
export async function translateSummary(summaryText: string): Promise<string> {
 ...
 */
export async function translateUserInput(japaneseText: string): Promise<string> {
...
export function estimateCost(operation: string, tokenCount: number): number {
  const costs = {
...
// Export for StreamingBuffer integration
...
export async function translateWithContext(
  text: string,
  context: GPT5TranslationContext
): Promise<string> {
